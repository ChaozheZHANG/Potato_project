...
def process_image_with_potato_detection(
    image_path: str,
    model: YOLO,
    rules: Dict[str, Any],
    ...
    r = results[0]
    img = cv2.imread(image_path) if save_viz else None
    ...
    if getattr(r, 'obb', None) is not None:
        ...
        for cls_id, conf_val, points, xywhr_val in zip(clses, confs, xyxyxyxy, xywhr):
            ...
            if class_name == "potato":
                # 这是土豆个体
                x, y, w, h = cv2.boundingRect(points.reshape((-1, 2)).astype(np.int32))
                potatoes_obb.append({
    ...
    for potato_info in potatoes_obb:
        potato_id = id_gen.generate()
        bbox = potato_info["bbox"]
        x, y, w, h = bbox
        
        # 找出属于这个土豆的所有缺陷（基于中心点在potato bbox内）
        potato_defects = []
        ...
        if img_viz is not None:
            ...
            cv2.polylines(img_viz, [potato_info["obb_points"].reshape((-1, 2)).astype(np.int32)], 
                         True, grade_color, 4)
            ...
            cv2.rectangle(img_viz, (x, y-label_h-20), (x+label_w+15, y), grade_color, -1)
            cv2.putText(img_viz, label, (x+7, y-10), font, font_scale, (255, 255, 255), thickness)
    ...
    if save_viz and img_viz is not None:
        ...
        info_text = f"Image: {Path(image_path).stem} | Potatoes: {len(potato_records)}"
        cv2.putText(img_viz, info_text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 0), 3)
...
def main():
    parser = argparse.ArgumentParser(description="使用potato标签识别土豆并分级")
    parser.add_argument("--model", required=True, help="训练好的模型权重（需包含potato类）")
    ...
    parser.add_argument("--out", default="/tmp/potato_inspection_system/results/potato_detection")
    ...
    for img_path in images:
        ...
        potato_records = process_image_with_potato_detection(
        ...
        if potato_records:
            ...
            for rec in potato_records:
                print(f"  └─ ID:{rec['potato_id']} | {rec['grade']} | 缺陷:{rec['total_defects']}个 | 面积:{rec['area']:.0f}px²")
